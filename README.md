...
| Call requests (chunk 6) | The site's primary action is `/request-a-call` (`components/agency/RequestCallForm.tsx`, en and es): name, mobile, year, best time; email and subjects optional. Posts to `/api/enquiries` with `preferred_contact = 'call'`. The owner gets the alert email plus a phone push through ntfy (`lib/notify.ts`, `NTFY_TOPIC` env; silent when unset). In Leads the enquiry shows a tap-to-call button, "Reached" and "No answer"; no answer (`PATCH /api/owner/enquiries/[id]` `{ action: 'no_answer' }`) counts the attempt and emails the family `buildCallbackMissedEmail` at most once every 20 hours. Copy and the promise live in `AGENCY.callBack` and `lib/enquiryCopy.ts` (`CALL_COPY_*`). Email is nullable on `enquiries` after `20260906_agency_chunk6.sql`; the API stores what the old table accepts until then and emails a phone-only lead to the owner rather than losing it |
| Subject tiers and IB (chunk 6) | `SUBJECTS` carries `tier` (`core`, `request`, `ib`), `group` and `yearLevels`; `subjectsForYearLevel()` drives both forms. Core is maths and the sciences; by-request subjects are matched only when a tested tutor exists; IB sits in the `ext2` rate band. Pages: `/subjects`, `/science-tutoring`, `/ib-tutoring`. Online prices are 75 / 85 / 95 since 6 Sep 2026 |
| Group classes (chunk 6) | `lib/classes.ts` (`GROUP_CLASSES`, `CLASS_RULES`, `TERM_1_2027`, price helpers) and `/classes`. A class registration is an ordinary enquiry: `/request-a-call?class=<key>` fixes the year, names the class in the message and `class_key`, and sets `source` to `class:<key>`. No booking system: the owner calls, the class runs at four students |
| Migration harness | `scripts/dev/migration-harness/run.sh <migration.sql> [more.sql ...]` runs each migration twice, in order, against a throwaway local PostgreSQL and exercises the triggers and constraints (`scenarios.sql`). Run it before handing a migration to the SQL editor |
...
| `NTFY_TOPIC` | Vercel (optional) | ntfy topic for phone push alerts on call requests and enquiries (`lib/notify.ts`). 6 to 64 chars of letters, digits, `_` or `-`; treat it like a password, anyone who knows it can read the alerts | Unset: no push, nothing fails. `NTFY_URL` overrides the server (default `https://ntfy.sh`) |
...
9. Everything under `supabase/migrations/` in filename order; the latest are `20260903_agency_enquiries_applications.sql` (enquiries, tutor_applications, tutor vetting columns), `20260904_agency_chunk2.sql` (agreement/vetting timestamps, late cancellations + view, incidents), `20260905_agency_chunk3.sql` (enquiry follow-up timestamps) `20260906_agency_chunk5.sql` (household credit ledger and invoice triggers, prepaid block columns, payment token default, referral and language columns on households, reviews, shared rate limiter) and `20260906_agency_chunk6.sql` (nullable enquiry email with a contact-present check, call-request columns, class_key). `~/LENIN_OS/05_CRESTIO/60_MIGRATIONS_TO_RUN/00_RUN_ALL_PENDING.sql` concatenates whatever production has not had yet.
