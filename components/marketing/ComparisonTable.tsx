...
function Cell({ value, side }: { value: CompareCell; side: 'own' | 'other' }) {
  if (value === 'yes') {
    return (
      <span className={['inline-flex', side === 'own' ? 'text-forest' : 'text-ink-muted'].join(' ')} role="img" aria-label="Included">
  ...
  if (value === 'no') {
    return (
      <span className="inline-flex text-claret" role="img" aria-label="Not included">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
  ...
  if (value === 'partial') {
    return (
      <span className="inline-flex text-amber-ink" role="img" aria-label="Partial">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
  ...
  return <span className="text-sm text-ink num tabular">{value}</span>;
}
