...
type Suggestion =
  | { kind: 'stale_student'; student_id: string; name: string; days_inactive: number }
      ...
      for (const s of (stale ?? []) as any[]) {
        ...
        if (!lastTs || lastTs < cutoff) {
          candidates.push({
            ...
            student_id: s.id,
            name: s.name,
            days_inactive: lastTs ? Math.floor((Date.now() - lastTs) / 86400_000) : 9999,
          });
        }
      }
          ...
          <li key={suggestionKey(s)} className="text-sm text-ink-muted flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              {s.kind === 'stale_student' ? (
                <>
                  <strong className="text-ink">{s.name}</strong> hasn't had a session in {s.days_inactive} days.
                </>
              ) : (
