...
export function buildPaymentReceiptEmail(args: Args): Built {
  ...
  const safeDate = escapeHtml(args.paidAtLabel);
  const cardLine = args.cardBrand && args.cardLast4
    ? `${args.cardBrand.toUpperCase()} ending ${args.cardLast4}`
    : 'Card payment';
  const safeCard = escapeHtml(cardLine);

  const subject = `Payment received - ${amount}`;

  const text =
    `Thanks for your payment.\n\n` +
    `${args.practiceName} received ${amount} on ${args.paidAtLabel}.\n\n` +
...
<body style="margin:0;padding:0;background-color:${CREAM};font-family:${FONT_BODY};color:${INK};">
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="560" style="max-width:560px;width:100%;background-color:${CREAM};">
        ...
        <tr><td style="padding:0 0 14px 0;">
          <p style="margin:0;font-family:${FONT_BODY};font-size:15px;line-height:1.65;color:${INK};">
            Thanks. ${safePractice} received <strong>${escapeHtml(amount)}</strong> on ${safeDate}.
          </p>
        </td></tr>
        <tr><td style="padding:8px 0 24px 0;">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border:1px solid ${RULE};border-radius:6px;">
            <tr>
              <td style="padding:14px 16px;font-family:${FONT_BODY};font-size:13px;color:${INK_MUTED};border-bottom:1px solid ${RULE};">Invoice</td>
              <td style="padding:14px 16px;font-family:${FONT_BODY};font-size:13px;color:${INK};text-align:right;border-bottom:1px solid ${RULE};">${safeInv}</td>
            </tr>
            <tr>
              <td style="padding:14px 16px;font-family:${FONT_BODY};font-size:13px;color:${INK_MUTED};border-bottom:1px solid ${RULE};">Date</td>
              <td style="padding:14px 16px;font-family:${FONT_BODY};font-size:13px;color:${INK};text-align:right;border-bottom:1px solid ${RULE};">${safeDate}</td>
            </tr>
            <tr>
              <td style="padding:14px 16px;font-family:${FONT_BODY};font-size:13px;color:${INK_MUTED};border-bottom:1px solid ${RULE};">Method</td>
              <td style="padding:14px 16px;font-family:${FONT_BODY};font-size:13px;color:${INK};text-align:right;border-bottom:1px solid ${RULE};">${safeCard}</td>
            </tr>
            <tr>
              <td style="padding:14px 16px;font-family:${FONT_BODY};font-size:14px;color:${INK};font-weight:600;">Amount</td>
              <td style="padding:14px 16px;font-family:${FONT_DISPLAY};font-size:18px;color:${INK};text-align:right;font-weight:600;letter-spacing:-0.01em;">${escapeHtml(amount)}</td>
            </tr>
          </table>
        </td></tr>
  ...
  // Force-reference FOREST so this constant remains in scope even if we later
  // reintroduce a button. (Linter clean.)
  void FOREST;
